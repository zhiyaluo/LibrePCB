...
public:
    ...
    PNS_LIBREPCB_PARENT_ITEM(const librepcb::project::BI_Footprint* fpt,
                             const librepcb::project::BI_FootprintPad *p) : footprint(fpt), pad(p)
    {
    }
...
public:
    ...
    const PNS_LIBREPCB_PARENT_ITEM *get_parent(const librepcb::project::BI_NetLine *line);
    const PNS_LIBREPCB_PARENT_ITEM *get_parent(const librepcb::project::BI_Via *via);
    const PNS_LIBREPCB_PARENT_ITEM *get_parent(const librepcb::project::BI_Footprint* fpt,
...
private:
    ...
    std::unique_ptr<PNS::SEGMENT> syncNetLine(const librepcb::project::BI_NetLine *line);
    std::unique_ptr<PNS::VIA> syncVia(const librepcb::project::BI_Via *via);
    std::unique_ptr<PNS::SOLID> syncPad(const librepcb::project::BI_Footprint* fpt,
                                        const librepcb::project::BI_FootprintPad *pad);
