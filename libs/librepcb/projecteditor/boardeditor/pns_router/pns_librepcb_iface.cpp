...
//                canvas->add_line(pts, seg_item->Width(), librepcb::ColorP::FROM_LAYER, layer_from_router(la)));
//    }
...
//        m_preview_items.insert(
//                canvas->add_line(pts, via_item->Diameter(), librepcb::ColorP::FROM_LAYER, layer_from_router(la)));
...
void PNS_LIBREPCB_IFACE::HideItem(PNS::ITEM *aItem)
{
    ...
    if (parent) {
        //if (parent->track) {
        //    librepcb::ObjectRef ref(librepcb::ObjectType::TRACK, parent->track->uuid);
        //    canvas->hide_obj(ref);
        //}
        //else if (parent->via) {
...
//std::pair<librepcb::BoardPackage *, librepcb::Pad *> PNS_LIBREPCB_IFACE::find_pad(int layer, const librepcb::Coordi &c)
//{
//    for (auto &it : board->packages) {
//        for (auto &it_pad : it.second.package.pads) {
//            auto pt = it_pad.second.padstack.type;
//            if ((pt == librepcb::Padstack::Type::TOP && layer == librepcb::BoardLayers::TOP_COPPER)
//                || (pt == librepcb::Padstack::Type::BOTTOM && layer == librepcb::BoardLayers::BOTTOM_COPPER)
...
//}
//
//librepcb::Junction *PNS_LIBREPCB_IFACE::find_junction(int layer, const librepcb::Coordi &c)
//{
